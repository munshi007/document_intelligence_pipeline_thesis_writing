...
All local inference tasks, including DocLayout-YOLO \cite{doclayout_yolo} detection, Qwen 2.5-VL \cite{qwen25} foundational extraction, and Llama-Vision 11B \cite{llama3_vision} specialist routing, were executed on a single workstation equipped with:
\begin{itemize}
...
\begin{enumerate}
    \item \textbf{Base TATR}: The Table Transformer \cite{tatr} applied directly to table crops.
    \item \textbf{GPT-4o (Zero-shot)}: Direct API extraction with a structured JSON prompt \cite{json_mode}.
    \item \textbf{The Ensemble Pipeline (Ours)}: DocLayout-YOLO + Semantic Pre-Masking + Dual-Student VLMs (Qwen 2.5-VL & Distilled Llama-Vision 11B).
\end{enumerate}
