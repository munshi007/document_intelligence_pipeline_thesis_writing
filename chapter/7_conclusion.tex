...
\begin{itemize}
    ...
    \item \textbf{Knowledge Distillation for Edge Architecture:} Through the application of LoRA \cite{lora} and the Unsloth framework \cite{unsloth}, the visual reasoning capabilities required for structural mapping were successfully distilled from a GPT-4o \cite{gpt4v} teacher directly into the Qwen2-VL architecture. This enables the high-performance 'Two-Pass Markdown Harvest' inference on consumer-grade hardware (RTX 3090), facilitating local deployment in secure enterprise environments without token ceiling bottlenecks.
