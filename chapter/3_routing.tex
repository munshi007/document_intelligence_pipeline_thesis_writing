...
\node[box, right=1.5cm of is_kv, fill=green!10] (vlm) {Specialist VLM\\(XML Prompting)};
...
\section{Specialist VLM Extractor and Hybrid XML-JSON Formatting}
\label{sec:vlm_specialist}

When the router encounters a structure that requires generative reasoning ($S_{\text{kv}} > K_{threshold}$), the region is passed to a Vision-Language Extractor \cite{gpt4v, qwen_vl}. Unlike general-purpose chat models, the extractor used in this pipeline combines strict XML-tagging for structural grid alignment with Pydantic JSON schemas \cite{pydantic} for final data validation.

\subsection{XML-Tag Prompt Engineering for Grid Alignment}
To mathematically force deterministic grid generation without JSON hallucination loops, the Local VLM Specialist uses a rigid XML-tagging hierarchy embedded within its system prompt. The model is forbidden from utilizing standard Markdown pipes ($|$), instead constrained to output sequential rows demarcated precisely by standard markup strings:
...
By constraining the text generation to these specific HTML-style closures, the extraction pipeline employs robust Regular Expression (RegEx) sweeping on the returned output. This ensures perfect programmatic mapping to the final structured Pydantic response \cite{pydantic}, preventing the deep nesting crashes common with raw JSON generation \cite{json_mode} in smaller models.
...
The coverage metric measures the ratio of source text tokens successfully mapped into the resulting structure. If a table contains forty numeric parameters but the VLM only extracts ten, the low coverage score triggers an automated failure.
